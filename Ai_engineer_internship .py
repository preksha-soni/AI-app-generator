...
Format:
{
  ...
  "key_entities": ["main data entities"]
    ...
    result = call_claude(system, f"Extract intent from: {user_prompt}")
...
Format:
{
  ...
  "integrations": ["payments", "email", etc]
    ...
    result = call_claude(system, f"Design architecture for:\nIntent: {json.dumps(intent)}\nOriginal: {user_prompt}")
    parsed = extract_json(result)
...
Format:
{
  ...
  "auth_schema": {
    ...
    result = call_claude(system, f"Generate schemas for:\nIntent: {json.dumps(intent)}\nArchitecture: {json.dumps(architecture)}")
    parsed = extract_json(result)
...
def stage4_validate_and_repair(schemas, intent, architecture):
    ...
    if errors:
        # Ask Claude to fix inconsistencies
        fix_prompt = f"""
...
def main():
    ...
    # Initialize session state key safely inside the Streamlit context
